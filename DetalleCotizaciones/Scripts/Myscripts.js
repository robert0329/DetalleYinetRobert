...
function LoadProduct(categoryDD) {
    $.ajax({
        ...
        success: function (data) {
            ...
            renderProduct($(categoryDD).parents('.mycontainer').find('select.Descripcion'), data);
        },
...
function renderProduct(element, data) {
    ...
    $.each(data, function (i, val) {
        $ele.append($('<option/>').val(val.ProductoID).text(val.Descripcion));
    })
}
...
$(document).ready(function () {
    ...
    $('#add').click(function () {
        ...
        var isAllValid = true;
        if ($('#DescripcionCategory').val() == "0") {
            isAllValid = false;
            $('#DescripcionCategory').siblings('span.error').css('visibility', 'visible');
        }
        else {
            $('#DescripcionCategory').siblings('span.error').css('visibility', 'hidden');
        }
        ...
        if (isAllValid) {
            ...
            $('.pc', $newRow).val($('#DescripcionCategory').val());
